...
export neptuneConfDir="${confDir}/neptune"
export cacheDir="$HOME/.cache/neptune"
...
[ -f "${neptuneConfDir}/hyde.conf" ] && source "${neptuneConfDir}/hyde.conf"
...
export wallbashDir="${neptuneConfDir}/wallbash"
export enableWallDcol
export wallpapersDir="${HOME}/.config/hypr/wallpapers"
...
set_conf()
{
    local varName="${1}"
    local varData="${2}"
    touch "${neptuneConfDir}/hyde.conf"

    if [ $(grep -c "^${varName}=" "${neptuneConfDir}/hyde.conf") -eq 1 ] ; then
        sed -i "/^${varName}=/c${varName}=\"${varData}\"" "${neptuneConfDir}/hyde.conf"
    else
        echo "${varName}=\"${varData}\"" >> "${neptuneConfDir}/hyde.conf"
    fi
}
